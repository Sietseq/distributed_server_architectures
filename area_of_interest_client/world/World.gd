...
func _on_check_despawn():
	# If most recent state does not have an existing player, despawn it
	for player in get_node("YSort/OtherPlayers").get_children():
		if world_state_buffer.back().has(int(player.name)) == false:
...
func _ready():
	...
	timer.connect("timeout", self, "_on_check_despawn")
