...
func _ready():
	...
	timer.connect("timeout", self, "_on_check_despawns")
...
func _physics_process(_delta):
	var render_time = ConnectionManager.current_server.client_clock - interpolation_offset
	...
	if world_state_buffer.size() > 1:
		...
		if world_state_buffer.size() > 2 :
			...
			if interpolation_factor > 2 || interpolation_factor < -2:
				return
			...
			for player in world_state_buffer[2].keys():
				...
				# Else spawn it
					...
					SpawnNewPlayer(player, world_state_buffer[2][player]["pos"])
